    ...
    return http
        .get<IHomeProject[]>("/api/projects", {
            transformResponse: [
                ...
                (response) => {
                    const projectResponse: IProjectsResponse = JSON.parse(
                        response
                    );
    ...
    return http
        .get<IProjectResponse>("/api/projects/" + id, {
            transformResponse: [
                // Manually map the response to a Typescript interface.
                (response) => {
                    const projectResponse: IGetProjectResponse = JSON.parse(
                        response
                    );
                    return projectResponse;
                },
...
async function createProject(project: ITempProject, subcategoryId: string) {
    ...
    async function uploadProject(result: any) {
        return await http.post<ICreateProjectResponse>(
            "/api/projects",
            result,
            {
                headers: {
                ...
                transformResponse: [
                    (response: any) => {
                        const projectResponse: IPostProjectResponse = JSON.parse(
                            response
                        );
                        return projectResponse;
                    },
...
async function getProjectCategories() {
    try {
        const cats = await http.get<ICategory[]>("/api/categories", {
            ...
            transformResponse: [
                (response: any) => {
                    const categoryResponse: ICategoryResponse = JSON.parse(
                        response
                    );
